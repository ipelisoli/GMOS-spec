# Reduces list of spectra, applies wavelength and flux calibration.
...
choice = input("Run gprepare? 1=yes, 0=no: ")
...
print ("=== Running flux calibration ===")

gmos.gscalibrate.unlearn()
gmos.gscalibrate(','.join(str(x) for x in list(science)), sfunction='../sens',
                 fl_ext='yes', extinction='onedstds$ctioextinct.dat')

science = list('c' + x for x in list(science))

print ("=== gscalibrate finished ===")

print ("If you're about to run for another spectrum, don't forget to clean-up your directory!")
print ("The code will not run as expected if a Flat.fits and/or QEcorr.fits files are already present.")
